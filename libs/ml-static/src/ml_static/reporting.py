...
def compute_predictions(
    ...
    links = np.concat(np.repeat(np.arange(link_number).reshape(1, -1), len(dataset), axis=0))
    # create a DataFrame
    df = pd.DataFrame(
        {
...
def compute_statistics(pred_df: pd.DataFrame) -> pd.DataFrame:
    ...
    total_true_flow = float(pred_df["true_value"].sum())
    wmape = (
        (pred_df["absolute_error"].sum() / total_true_flow) * 100 if total_true_flow > 0 else np.nan
    )
    ...
    # Flow-weighted metrics (keep simple):
    # - flow-weighted share within 10% relative error (true flow > 10)
    # - flow-weighted share with GEH < 5 (true flow > 10)
    flow_share_cutoff = 10.0
    baseline_mask = pred_df["true_value"] > flow_share_cutoff
    baseline_total = float(pred_df.loc[baseline_mask, "true_value"].sum())

    if baseline_total <= 0 or baseline_mask.sum() == 0:
        flow_weighted_within_10 = np.nan
        flow_weighted_geh_under_5 = np.nan
    else:
        within_10_mask = baseline_mask & (pred_df["absolute_percentage_error"] <= 10.0)
        flow_weighted_within_10 = (
            float(pred_df.loc[within_10_mask, "true_value"].sum()) / baseline_total
        ) * 100

        geh_under_5_mask = baseline_mask & (pred_df["geh"] < 5)
        flow_weighted_geh_under_5 = (
            float(pred_df.loc[geh_under_5_mask, "true_value"].sum()) / baseline_total
        ) * 100

    # Normalized RMSE (NRMSE) computed in the flow space:
    # RMSE normalized by the mean true flow value for interpretability.
    mean_true_flow = pred_df["true_value"].mean()
    nrmse = rmse / mean_true_flow if mean_true_flow > 0 else np.nan
    ...
    stats = {
        ...
        "F in 10%": flow_weighted_within_10,
        "F in GEH<5": flow_weighted_geh_under_5,
    }
...
def plot_performance_diagnostics(
    ...
    def _make_binned_series(series: pd.Series, n_bins: int, method: str = "quantile"):
        """
        ...
        if method == "quantile":
            ...
            raise ValueError("Unsupported binning method. Choose 'quantile' or 'fixed'.")
    ...
    if color_series is None or color_series.dropna().empty:
        ...
        df_plot = df_plot.dropna(subset=["true_value", "prediction", "_color_val"])
        if len(df_plot) == 0:
            axes[1].scatter(pred_df["true_value"], pred_df["prediction"], alpha=0.5)
        else:
            binned, labels = _make_binned_series(df_plot["_color_val"], bins, method=binning)
            df_plot["_bin"] = binned
            present_bins = sorted(df_plot["_bin"].dropna().unique().astype(int).tolist())
    ...
    if color_series is None or color_series.dropna().empty:
        ...
        if len(df_plot2) == 0:
            ...
            binned2, labels2 = _make_binned_series(df_plot2["_color_val"], bins, method=binning)
            df_plot2["_bin"] = binned2
            present_bins2 = sorted(df_plot2["_bin"].dropna().unique().astype(int).tolist())
...
def plot_splits_diversity(
    ...
    for split_name in ["train", "val", "test"]:
        ...
        print(f"  Filtering Noise (Mean Flow < {min_flow_thresh}): Kept {n_kept}/{n_total} links")
    ...
    if plot:
        fig, axs = plt.subplots(1, 4, figsize=(24, 6))  # Aumento un po' l'altezza
        ...
        for i, ax in enumerate(axs):
            ...
            # Soglie visive
            if key == "link_cv_cap":
                ax.axvline(0.1, color="r", linestyle="--", label="Threshold", alpha=0.5)
